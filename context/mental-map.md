...
| ui-v2/ (React) | **The frontend becoming the product** (branch `feature/dashboard-v2`, since 2026-07-29): React 19 + Vite + Tailwind + Recharts, `src/data.ts` (`useLiveData` polls `/api` ×3 indices every 5s, retains raw payloads for replay, returns `dead[]`) + `src/App.tsx` (ANSWER band, replay transport, FOCUS feed, real trade validator, token capture). Also `src/trade/` (Trade tab — CandL chart mount, MAP-levels overlay, indicator reshaping, all built on `/api/data` + `/api/chain`, no new route) and `src/vendor/candl/` (CandL Charts engine, Apache-2.0, pinned @5389381, source-compiled — **pristine, never edited**; see `ui-v2/src/vendor/VENDOR.md`). File-level separate from `ui/` — neither imports the other. See `context/ui-v2-dashboard.md`. |
