...
| C13 | **The fixed 30-minute exit is REFUSED.** §1's management table measured it (*"no stop, fixed 30-min exit"*, +9.3/72%) and it was **not adopted** — it has no stop, so it is not a trade. The adopted management is *"hold the stop until VWAP, then trail band-to-band; from +2σ, 15 pts under the high. Flat by 15:15."* The operator refused the 30-minute exit before 2026-08-08 and it was never written down, so it came back twice — in §5d's backtest bar and §5e's forward bar. **Any scoring horizon must be the adopted management, not a clock** | operator |
...
| ▶ | Hover callout does not call a rejected band "apna setup" (A1) | **DEFERRAL LIFTED 2026-08-08** — the operator is redesigning the callout and the event vocabulary themselves next session, so it is IN scope again. Original note kept: was deferred 2026-08-07 — *"dont touch the dynamic call out i am thinking of modifying it in some way but later"*. It now reads `rotation_run`, so it shows the entry's own receipt; the wording is theirs to redesign |
