...
- **Test count is now 707** (`pytest -q`, 2026-08-20 - 699 plus eight in `test_pools.py` for wall-vs-cluster and the Kite order-count match; before that 699, 2026-08-20 - 689 plus ten in `test_chainside.py`, which supplies the fuel and drain the order book cannot see, completing the three conditions; before that 689, 2026-08-20 - 663 plus sixteen in `test_fuse.py` (the seam turning detector magnitudes into a regime verdict) and ten in `test_pools.py` (book terrain: shelves and vacuums); before that 663, 2026-08-20 - 639 plus twelve in `test_drag.py` (the honest drag meter, real delta instead of an assumed 0.5) and twelve in `test_regime.py` (the PIN/TRANSITION/CASCADE gate, shadow-only); before that 639, 2026-08-20 - 637 plus two pinning the per-day senses log; before that 637, 2026-08-20 - 609 plus ten in `test_absorption.py`, nine in `test_depth_pull.py` and nine in `test_senses.py`, the book detectors and the wiring that forward-logs them; before that 609, 2026-08-20 - 599 plus ten in `test_sweep.py`, the depth-consumption detector born against the live frame; before that 599, 2026-08-20 - 595 plus four pinning the depth ladder against a live frame captured 09:56 IST (`data/feed_frame_2026-08-20.json`); before that 595, 2026-08-20 — 587 plus eight in `test_upstox_adapter.py` for the depth ladder; before that 587 — 581 plus six in `test_operator_objects.py`, the trapped-inventory ledger and forcing detector wired in beside `chain_metrics.py`; before that 581 — 575 plus the zone-scorer pin in `test_zone_state.py` and five in `test_recover_tape.py`; before that 575, 2026-08-15 evening — 569 plus six
